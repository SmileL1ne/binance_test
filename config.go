...
func (c *config) getConfig() error {
	...
	// ошибаешься - не всегда так - тут у нас запросы (т.е io нагрузка), но если бы ты просто проводил вычисления, где не надо ждать ответа от сети - то больше горутин работало бы медленее
